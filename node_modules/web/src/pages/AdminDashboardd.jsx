...
/* ========================== Helpers de formato ========================== */
// yyyy-mm-dd desde Date o string
...
// yyyy-mm-dd, pero **en horario local** (corrige UTC)
...
// Detecta la fecha en el registro (varias keys y formatos)
function getRecordISODate(r) {
  if (!r) return "";
  ...
  if (typeof v === "number") {
    ...
    return toLocalISODate(new Date(ms));
  }
  ...
  if (typeof v === "string") {
    ...
    if (!Number.isNaN(maybe.getTime())) {
      return toLocalISODate(maybe);
    }
  }

  // Último intento: si viene Date
  if (v instanceof Date && !Number.isNaN(v.getTime())) {
    return toLocalISODate(v);
  }
...
function getFincaLabel(r) {
  const v =
    ...
    r?.locationId ??
    r?.location ??
    r?.crewId ??       // 👈 clave de tu API
    r?.crew_id ??      // 👈 clave de tu DB/joins
    "-";

  if (v === "-" || v == null) return "-";
  const s = String(v);
  const match = s.match(/\d+/);         // si viene "Cuadrilla 2" -> "2"
...
export default function AdminDashboard() {
  ...
  const [day, setDay] = useState(toISODate(new Date())); // YYYY-MM-DD
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true);
      try {
        // TIP: si querés filtrar desde backend, agregá &date=${day}
  ...
  // Filtrado: por día (usando fecha detectada robusta) + búsqueda
const filteredRecent = recent
  .filter((r) => {
    if (!day) return true;
    const raw = r?.date ?? r?.fecha ?? "";                // usa date si viene del server
    if (!raw) return false;
  ...
  return (
    <div className="min-h-screen p-6">
      ...
      {/* Registros recientes (por día local) */}
      <Card>
        <CardHeader className="pb-2">
                ...
                <TableBody>
                  {filteredRecent.length === 0 ? (
                    ...
                    filteredRecent.map((r, i) => {
                      const recIso = getRecordISODate(r);
                      const recDate =
                        recIso
                          ? recIso.split("-").reverse().join("/")
                          : "-";
                      return (
                        <TableRow key={r.id ?? i} className="hover:bg-white/5">
                          ...
                          <TableCell>
                            {r.date
                              ? r.date.split("-").reverse().join("/")  // viene ISO desde el server
                              : (r.fecha
